<template>
  <div
    role="img"
    :aria-label="`Activity last ${DEFAULT_ACTIVITY_DAYS} days`"
    class="absolute right-0 bottom-0 z-20"
  >
    <div class="relative z-10">
      <div
        class="flex items-end gap-0.5 h-20 w-44 max-w-72"
...
import { DEFAULT_ACTIVITY_DAYS } from '~~/constants/activity'
...
async function fetchActivity() {
  ...
  maxValue.value = Math.max(1, ...activityDays.map(d => d.count))

  bars.value = activityDays.map(day => ({
    height: 0,
    value: day.count,
